  ...
  .then(data => {
    ...
    if (path.includes("index.html") || path === "/" || path.endsWith("/")) {
      ...
      if (wwdDesc && homeData.WhatWeDoSection) {
        ...
        document.getElementById("Email").innerHTML = `<i class="fa-light fa-circle-envelope me-2"></i>${contact.Email}`;
        document.getElementById("Phone").innerHTML = `<i class="fa-light fa-circle-phone me-2"></i>${contact.Phone}`;
    ...
    if (path.includes("about.html")) {
      ...
      if (aboutThumbnail) aboutThumbnail.textContent = aboutData.Thumbnail.Title;

      const whoWeAreCol = document.querySelector("section.container-fluid.my-5 .col-lg-6");
      if (whoWeAreCol) {
        const branchesHTML = aboutData.WhoWeAre.Branches.map(loc => 
      ...
      const missionP = document.querySelector(".fa-rocket")?.closest(".d-flex")?.querySelector("p:last-child");
      const visionP = document.querySelector(".fa-telescope")?.closest(".d-flex")?.querySelector("p:last-child");
      ...
      const valuesRow = document.querySelector("section.container-fluid.my-5.bg-white .row.g-4");
      if (valuesRow) {
        valuesRow.innerHTML = "";
...
// --- CONTACT PAGE SECTION ---
  if (path.includes("contact.html")) {
      ...
      if (contactInfoDiv && contactData.Addresses) {
          ...
          const addressTitle = Array.from(contactInfoDiv.querySelectorAll("h1")).find(h1 => h1.textContent === "Address");
          
          if (addressTitle) {
              // Generate HTML for all addresses in the JSON array
              const addressHTML = contactData.Addresses.map(addr => `
                  <p class="location">
